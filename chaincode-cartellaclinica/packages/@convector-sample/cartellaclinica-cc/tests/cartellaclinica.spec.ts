...
import { CartellaclinicaaleController, Cartellaclinicaale } from '@convector-sample/cartellaclinicaale-cc';

import { Cartellaclinica, CartellaclinicaController, Attribute } from '../src';

describe('Cartellaclinica', () => {
  chai.use(chaiAsPromised);
  // A fake certificate to emulate multiple wallets
  const fakeSecondCartellaclinicaaleCert = '-----BEGIN CERTIFICATE-----' +
  ...
  let cartellaclinicaaleCtrl: ConvectorControllerClient<CartellaclinicaaleController>;
  let cartellaclinicaId = '1-1002-1002'
  before(async () => {
    ...
    await adapter.init([
      ...
      {
        version: '*',
        controller: 'CartellaclinicaaleController',
        name: join(__dirname, '../../cartellaclinicaale-cc')
      }
    ]);
    adapter.stub['fingerprint'] = mockIdentity;
    cartellaclinicaCtrl = ClientFactory(CartellaclinicaController, adapter);
    cartellaclinicaaleCtrl = ClientFactory(CartellaclinicaaleController, adapter);
  });

  it('should try to create a cartellaclinica but no government identity has been registered', async () => {
    const cartellaclinicaSample = new Cartellaclinica({
      id: cartellaclinicaId,
      firstname: 'Walter',
      lastname: 'Montes',
    });

    await expect(cartellaclinicaCtrl.create(cartellaclinicaSample)).to.be.eventually
      .rejectedWith(Error);
  });

  it('should create the government identity', async () => {
    await cartellaclinicaaleCtrl.register('gov', 'Big Gov');

    const justSavedModel = await adapter.getById<Cartellaclinicaale>('gov');

    expect(justSavedModel.id).to.exist;
  });

  it('should create a cartellaclinica', async () => {
    const cartellaclinicaSample = new Cartellaclinica({
      id: cartellaclinicaId,
      firstname: 'Walter',
      lastname: 'Montes',
    ...
    await cartellaclinicaCtrl.create(cartellaclinicaSample);

    const justSavedModel = await adapter.getById<Cartellaclinica>(cartellaclinicaSample.id);
  ...
  it('should add a birth-year attribute through the gov identity', async () => {
    ...
    await cartellaclinicaCtrl.addAttribute(cartellaclinicaId, attribute);

    const justSavedModel = await adapter.getById<Cartellaclinica>(cartellaclinicaId);
    expect(justSavedModel.id).to.exist;
  ...
  it('should create a cartellaclinicaale for the MIT', async () => {
    // Fake another certificate for tests
    (adapter.stub as any).usercert = fakeSecondCartellaclinicaaleCert;

    await cartellaclinicaaleCtrl.register('mit', 'MIT - Massachusetts Institute of Technology');

    const justSavedModel = await adapter.getById<Cartellaclinicaale>('mit');

    expect(justSavedModel.id).to.exist;
  });

  it('should try to create a cartellaclinica but the MIT cannot', async () => {
    const cartellaclinicaSample = new Cartellaclinica({
      id: cartellaclinicaId + '1111',
      firstname: 'Walter',
      lastname: 'Montes'
    });
  ...
  it('should add a mit-degree attribute through the MIT identity', async () => {
    ...
    const cartellaclinica = await adapter.getById<Cartellaclinica>(cartellaclinicaId);
    expect(cartellaclinica.id).to.exist;

    const resultingAttr = cartellaclinica.attributes.find(attr => attr.id === attributeId);
    expect(resultingAttr).to.exist;
    expect(resultingAttr.id).to.eq(attribute.id);
  });
  ...
  it('should try to make the birth-year expire! with the MIT identity', async () => {
    ...
    (adapter.stub as any).usercert = fakeSecondCartellaclinicaaleCert;
    const cartellaclinica = await adapter.getById<Cartellaclinica>(cartellaclinicaId);
    let attribute = cartellaclinica.attributes.find(attr => attr.id === 'birth-year');

    attribute.certifierID = 'mit';
    attribute.expired = true;
    await expect(cartellaclinicaCtrl.addAttribute(cartellaclinicaId, attribute)).to.be.eventually.rejectedWith(Error);
  });

  it('should get all the people', async () => {
    let res = await cartellaclinicaCtrl.getAll();
    console.log(JSON.stringify(res));
    expect(res.length).to.be.eq(1);
  });

  it('should get all the people with the birth-year 1993', async () => {
    let res = <Cartellaclinica[]>(await cartellaclinicaCtrl.getByAttribute('birth-year', '1993'));
    console.log(JSON.stringify(res));
    expect(res.length).to.be.eq(1);
  });

  it('should get zero people with the birth-year 1990', async () => {
    let res = <Cartellaclinica[]>(await cartellaclinicaCtrl.getByAttribute('birth-year', '1990'));
